<template>
  <div v-if="isVisible" class="fixed inset-0 bg-slate-900 bg-opacity-80 flex justify-center items-center z-50">
    <div class="bg-white p-12 rounded-lg w-11/12 lg:w-auto relative text-center">
      <h3 class="lg:text-2xl text-2xl font-bold text-blue">Excluir esta pergunta</h3>
      <p class="text-[#4D5E77] pt-3">Tem certeza que deseja excluir esta pergunta?</p>
      ...
      <div class="flex justify-between gap-5">
        ...
        <button @click="confirmDelete"
          class="w-full lg:w-auto mt-6 flex justify-center items-center gap-2 rounded-lg p-3 bg-[#E83F5B] font-medium text-md text-white transition hover:brightness-95 focus:outline-none">
          Sim, excluir
        </button>
      </div>
    </div>
...
const emit = defineEmits(['close', 'deleteQuestion']);
...
const confirmDelete = () => {
  ...
  emit('deleteQuestion', { question: props.question, password: password.value });
  closeModal();
